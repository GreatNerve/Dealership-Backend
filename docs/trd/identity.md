...
| --- | --- | --- | --- | --- |
| POST | `/auth/register` | none | email, optional `name` (max 100), password, role `CUSTOMER` or `DEALERSHIP_STAFF` | 201 User (`id`, `email`, `name`, role, nested customer / home shop) |
| POST | `/auth/login` | none | JSON `{ email, password }` **or** form `username` (email) + `password` (Swagger Authorize / OAuth2 password) | JSON 200 envelope `data`: `{ access_token, token_type, expires_in }`. Form 200 `{ access_token, token_type, expires_in }` (unwrapped for Swagger). JWT **7 days** (`APP_JWT_TTL`), no refresh in v1 |
| GET | `/me` | JWT | — | Current User + role + optional `name`. Nested `customer` (`id`, `contact`, optional `name`) when the User is a Customer. Nested `homeDealership` (`id`, name, timezone, address) when Staff has a shop. Ids `customerId` / `homeDealershipId` stay. |
