...
| GET | `/notifications` | Staff, home Dealership (`notifications.dealership_id`), else empty page not other shops’ rows. Paginated + `q`. Query: Instant `from`/`to` (inclusive/exclusive on `created_at`; frontend computes Dealership-local “today”; `from` must be **strictly before** `to`), `status` (worker enum, not `NOT_SCHEDULED` — that value is 400), `generation`, `channel`, `appointmentId`, `hasEvent` (`OPENED` \| `DELIVERED` \| `SOFT_BOUNCE` \| `HARD_BOUNCE` \| `CLICKED` \| `SPAM` \| `BLOCKED` \| `ERROR` \| `ACCEPTED`). Derived open/bounce = `EXISTS` on events, not columns. Nested **Appointment** (customer name, Vehicle Number, visit time) is batched `findAllById` after the page — not the Appointment UUID as the only UI field. Subject/body omitted on the list. |
| GET | `/notifications/stats` | Staff, home Dealership. Query Instant `from`/`to` required. Optional `bucket` (`DAY` \| `WEEK` \| `MONTH`). JSON: `appointments` (count with `scheduled_at` in range at that shop), `notificationsSent` (`sent_at` in range), `failed` (`DEAD_LETTER` by `updated_at`), `opened` / `softBounce` / `hardBounce` / `bounced` (`EXISTS` distinct Notification with that event type and `occurred_at` in range; `bounced` is either bounce), `buckets[]` (zero-filled `date` / `sent` / `failed` / `bounced` / `opened` when `bucket` is set). Bounce/open buckets use the **first** `occurred_at` in range per Notification so summing days equals the distinct total. Register this mapping **before** `/{id}`. |
| GET | `/notifications/{id}` | Staff, home Dealership, else 404. Worker fields + `channel`, `generation`, `dealershipId`, `appointmentId`, nested Appointment (same shape as list), optional `reminderId` / `offsetMinutes`. **Manual** includes `subject`/`body`. `events`: append-only timeline (`eventType`, `occurredAt`, `provider`). |
| POST | `/appointments/{id}/notifications` | Staff, home Dealership, else 404. `Idempotency-Key` required (same table as Appointment create; fingerprint includes Appointment id + subject + body). Body `{ "subject": "…", "body": "…" }` (required, sanitized). Creates `generation=MANUAL`, `channel=EMAIL`, `dealership_id` from the Appointment, `reminder_id` null, stores subject/body, writes `MANUAL_NOTIFICATION` outbox. 202 + Notification id. Same worker pipeline. `400` blank subject/body or missing key. |
...
| POST | `/webhooks/delivery/{provider}` | Public. `APP_DELIVERY_WEBHOOK_SECRET` in `Authorization` (`Bearer …`, `Token …`, or the raw secret for Brevo Token auth), compared constant-time. `{provider}` selects `DeliveryWebhookAdapter` (`brevo`, `stub`). A JSON **array** is each element (max **100**, else `400 VALIDATION_ERROR`); a JSON object is one event. Map payload → generic `DeliveryEventType`. Resolve Notification by Correlation Key (UUID in mapped custom header field). One transaction: load known ids, insert new rows. `provider_event_id` longer than 255 is SHA-256 hex (`Inputs.fit`); `raw_type` clipped to 64. Unique `(notification_id, provider, provider_event_id)` → skip duplicate (200). Unknown Notification → 204 no insert (do not create; 2xx so the provider stops retrying). Unmapped event type → 204 no insert (or `OTHER` + `raw_type` if the adapter chooses to keep debug). **Do not** change worker `Notification.status`. Do not log recipient email. Rate-limited per endpoint (IP). |
