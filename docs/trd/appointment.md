...
| POST | `/appointments/{id}/reschedule` | Body `{ scheduledAt }`. Customer: own. Staff: home Dealership. New UTC Instant and **Booking Offset** from that value. Reminders own Schedule Version (`MAX(schedule_version)+1` on insert). Other customer / other shop → 404. 409 if not Confirmed. Concurrent write → `409 CONCURRENT_UPDATE`. |
