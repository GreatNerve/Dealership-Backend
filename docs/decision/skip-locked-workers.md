...
**Worker pool: 2–4** (default **2**, configurable). Email is slow, so more than one worker can send at once — but not 16 workers just because the laptop has 16 threads. SMTP (Brevo) is the limiter; uniqueness still comes from the DB key. Each consumer `prefetch=1` so a slow send does not prefetch-and-stall extra messages. Mail body comes from the outbox snapshot, not a second full-graph load.
