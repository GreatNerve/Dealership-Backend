...
Send window: **adjacent gap ÷ 2**, not a buffer hour and not the full stretch to the next offset. `nextDueAt` = next smaller offset due, or visit start. Send while `dueAt <= now() < dueAt + (nextDueAt − dueAt)/2`. Default `24h,2h`: 24h sends **T−24h → T−13h** (half of 22h); 2h sends **T−2h → T−1h** (half of 2h). Remaining to next due greater than half the gap → send; at or past midpoint → `EXPIRED`. Booked at T−25h, down at T−24h, back at T−23h or T−22h → **send 24h**. Back at T−12h → 24h no. 2h recovered at T−90m → send; at T−30m → no. **Insert uses the same midpoint** so a first book at T−20h is still `PENDING`. Skip that offset again only when a System Notification for it is already SENT and the new due is past (reschedule inside 24h after they already got the 24h). Cancel and reschedule cancel old rows so a worker must re-check and must not send.
