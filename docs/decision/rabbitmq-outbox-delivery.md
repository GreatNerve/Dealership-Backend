...
When a Reminder is **due**, the poller claims it and writes an **outbox row in that claim transaction**, payload = **lean snapshot** (not a full entity dump). A publisher pushes to **RabbitMQ**, then a consumer sends from that snapshot. Appointment create does **not** write outbox rows.
