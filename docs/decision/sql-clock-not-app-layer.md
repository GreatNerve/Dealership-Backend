...
After a claim, replicate a **lean snapshot** into the outbox payload (ids, `scheduled_at`, `display_offset`, shop name, optional customer name, vehicle make/model/year + Vehicle Number, contact). The mail worker formats that snapshot. It does not hydrate entities again.
