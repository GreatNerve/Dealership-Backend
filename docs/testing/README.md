...
| End-to-end | [end-to-end.md](end-to-end.md) | HTTP → Appointment → Reminders → due work → stub send → DB. |
...
**Must pass before the video:** uniqueness + one happy-path e2e (create, Reminder rows for configured offsets, stub invoked once per type).
