...
- Mail body uses **Booking Offset** as local wall time only (`Tuesday, 22 September 2026` / `10:00 PM`). No “UTC”, no offset in the mail. Vehicle make/model/year and **Vehicle Number**. No “2-hour reminder”. HTML + plain text. From `APP_MAIL_FROM` (`dheeraj@greatnerve.com`). Staff GET is **Dealership Timezone**. No Customer timezone field. See [../trd/time.md](../trd/time.md).
