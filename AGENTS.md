...
- JWT access token 7 days. `APP_JWT_SECRET` must be at least 32 bytes (start refuses to pad). CORS allows all origins (`APP_CORS_ORIGINS=*`). Idempotency-Key TTL 24h. Expired `idempotency_keys` purged at UTC midnight. Notification `idempotency_key` is not TTL-purged.
