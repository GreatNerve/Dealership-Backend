...
- Staff `GET /notifications` is the shop-wide list (home Dealership via `notifications.dealership_id`): paginated and searchable (`q` is customer name / Vehicle Number), Instant `from`/`to` (`from` strictly before `to`; frontend computes Dealership-local “today”), `status` (not `NOT_SCHEDULED`), `generation`, `channel`, `appointmentId`, optional `hasEvent`. Nested Appointment (customer, Vehicle, visit time) on list and item GET. `appointmentId` list items include the **Delivery Event** timeline. `GET /notifications/{id}` includes the timeline, latest `occurredAt` first. `GET /dashboard/stats?from&to` omits `bucket` for totals (`buckets[]` empty); optional `bucket=DAY|WEEK|MONTH` zero-fills slices in shop TZ (max 400). Staff UI: calendar-year totals (1 Jan–31 Dec shop TZ) without `bucket`, last 7 days with `bucket=DAY`. `bounced` includes `BLOCKED`. Resource `GET /notifications/stats` and `GET /appointments/stats` stay for totals (optional `bucket`). Stats CTE filters healed `occurred_at` to `[from, to)` (epoch ≥ 1e12 as millis). `POST /appointments/{id}/notifications` is **Manual** send (`subject` + `body`, `Idempotency-Key` required); templates live in the frontend; SMTP wraps body in the same HTML shell as System mail. Customer does not list Notifications.
...
- Reuse: one implementation per job (page, 404, time format, Instant `from`/`to`, `StatsBucket`, Reminder SQL, sender, `DeliveryWebhookAdapter`, `ApiErrorCode`, `Inputs` sanitize, `ClientIps`). No copy-paste. See [docs/trd/code-style.md](docs/trd/code-style.md).
