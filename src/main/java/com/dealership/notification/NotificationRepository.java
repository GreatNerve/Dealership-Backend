  ...
  @Query(
      value =
          ...
          SELECT
            ...
            (SELECT count(DISTINCT e.notification_id)
              FROM notification_delivery_events e
              JOIN notifications n ON n.id = e.notification_id
              WHERE n.dealership_id = :shop AND e.event_type = 'OPENED'
                AND e.occurred_at >= :fromTs AND e.occurred_at < :toTs),
            (SELECT count(DISTINCT e.notification_id)
              FROM notification_delivery_events e
              JOIN notifications n ON n.id = e.notification_id
              WHERE n.dealership_id = :shop AND e.event_type = 'SOFT_BOUNCE'
                AND e.occurred_at >= :fromTs AND e.occurred_at < :toTs),
            (SELECT count(DISTINCT e.notification_id)
              FROM notification_delivery_events e
              JOIN notifications n ON n.id = e.notification_id
              WHERE n.dealership_id = :shop AND e.event_type = 'HARD_BOUNCE'
                AND e.occurred_at >= :fromTs AND e.occurred_at < :toTs),
            (SELECT count(*) FROM notifications n
            ...
            (SELECT count(DISTINCT e.notification_id)
              ...
              WHERE n.dealership_id = :shop
                AND e.event_type IN ('SOFT_BOUNCE', 'HARD_BOUNCE')
                AND e.occurred_at >= :fromTs AND e.occurred_at < :toTs)
          """,
      nativeQuery = true)
  ...
  @Query(
      value =
          ...
          SELECT date_trunc(CAST(:bucketUnit AS text), x.first_at AT TIME ZONE d.timezone)::date,
                 ...
                 count(*)
          FROM (
            SELECT e.notification_id, n.dealership_id, min(e.occurred_at) AS first_at
            FROM notification_delivery_events e
            JOIN notifications n ON n.id = e.notification_id
            WHERE n.dealership_id = :shop
              AND e.event_type IN ('SOFT_BOUNCE', 'HARD_BOUNCE')
              AND e.occurred_at >= :fromTs AND e.occurred_at < :toTs
            GROUP BY e.notification_id, n.dealership_id
          ) x
